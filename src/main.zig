...
fn isCounterClockWise(poly: []const Point) bool {
    var seq: f32 = 0;
    for (poly) |a, idx| {
...
///          0: on the line
///          1: left side
fn pointLineDirection(a: Point, b: Point, p: Point) i2 {
    const nom = (p.y - a.y) * (b.x - a.x);
    const dom = (p.x - a.x) * (b.y - a.y);

    if (std.math.absFloat(nom - dom) < ROUNDERR) {
        return 0;
...
///          0: on
///          1: inside
fn pointInPolygon(p: Point, poly: []const Point) i2 {
    var wm: i32 = 0;
    for (poly) |a, idx| {
        const b = poly[(idx + 1) % poly.len];
